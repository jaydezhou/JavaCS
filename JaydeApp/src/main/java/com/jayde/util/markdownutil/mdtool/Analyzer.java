...
import com.jayde.util.markdownutil.mdtool.bean.TextLinePiece;
import com.jayde.util.markdownutil.mdtool.bean.ValuePart;
import com.jayde.util.markdownutil.mdtool.filter.*;
...
public class Analyzer {
    ...
    private static List<ValuePart> text2ValuePart(String text, List<String> notCheckMDTokens,
        ...
        if (thirdPartEndIndex > 0) {
            ...
            String thirdPart = "";
            if (piece.getPieceType() == TextLinePiece.PieceType.IMAGE) { // image的开始符是两个字符，结束符是一个字符，所以要特殊处理
