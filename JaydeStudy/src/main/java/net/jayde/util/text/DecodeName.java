...
public class DecodeName {
    ...
    private static String oldXing = "", oldMing = "";
    private static String newXing = "", newMing = "";
    private static String[] XINGS = {
            "李","王","张","刘","陈","杨","赵","黄","周","吴","徐","孙","胡","朱","高","林","何","郭","马","罗","梁","宋","郑","谢","韩","唐","冯","于","董","萧","程","曹","袁","邓","许","傅","沈","曾","彭","吕","苏","卢","蒋","蔡","贾","丁","魏","薛","叶","阎","余","潘","杜","戴","夏","钟","汪","田","任","姜","范","方","石","姚","谭","廖","邹","熊","金","陆","郝","孔","白","崔","康","毛","邱","秦","江","史","顾","侯","邵","孟","龙","万","段","漕","钱","汤","尹","黎","易","常","武","乔","贺","赖","龚","文"};

    private static String[] XINGS_BAK = {
    ...
    private static String decodeName(String name) {
        oldName = name.trim();
        oldXing = "";
        oldMing = oldName.substring(1);
        String tmpXing = oldName.substring(0, 1);
        String tmpMing = oldName.substring(1);
        for (int i = 0; i < XINGS.length; i++) {
            if (tmpXing.equals(XINGS[i])) {
                oldXing = XINGS[i];
                newXing = XINGS[XINGS.length - i - 1];
                break;
            }
        }
        if (oldXing.equals("")) {
            char[] cs = tmpXing.toCharArray();
            int ics = 0;
            for (char c : cs) {
                ics += c;
            }
            ics = ics % (XINGS.length);
            oldXing = tmpXing;
            newXing = XINGS[ics];
        }
        char[] ms = oldMing.toCharArray();
        int ims = 0;
        for (char c : ms) {
            ims += c;
        }
        ims = ims % (MINGS.length);
        newMing = MINGS[ims];
        return newXing + newMing;
    }

    public static void main(String[] args) {
        System.out.println(DecodeName.decodeName(" z粂张a三℃"));
    }
}
