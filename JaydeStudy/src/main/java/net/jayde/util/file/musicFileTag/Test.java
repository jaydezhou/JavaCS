...
public class Test {
    ...
    private static final int CMP4TAGATOM_TRACK = 7; // 曲目
    ...
    public static void main(String[] args) throws Exception {
        RandomAccessFile f = new RandomAccessFile("d:\\music\\java\\alac_all.m4a", "r");
        ...
        do{
            ...
            String ignores[] = {"ftyp","mvhd"};//不需要解析的标记，跳过整个块
//            String ignores[] = {};//不需要解析的标记，跳过整个块
            if(isInArray(ignores, tag)){
                f.skipBytes(tagSize-8); continue;
            }
//            String parents[] = {"moov","udta","ilst"};
            ...
            else if("trak".equals(tag)){
                System.out.println("这里是trak");
    ...
    private static void HandleRealDataBuf(int atomID, byte[] pRealBuf) {
        ...
        switch (atomID)
        {
            case CMP4TAGATOM_ALBUM: // 专辑
                System.out.println("专辑："+new String(pRealBuf));
                break;
            case CMP4TAGATOM_ARTIST: // 艺术家
                System.out.println("艺术家："+new String(pRealBuf));
                break;
            case CMP4TAGATOM_NAME: // 名称
                System.out.println("名称："+new String(pRealBuf));
                break;
            case CMP4TAGATOM_DATE: // 日期
                System.out.println("日期："+new String(pRealBuf));
                break;
            case CMP4TAGATOM_GENRE: // 流派
                System.out.println("流派："+new String(pRealBuf));
                break;
            case CMP4TAGATOM_COVER: // 封面
                break;
            default:
                System.out.println("其他："+new String(pRealBuf));
                break;
        }
    }
}
